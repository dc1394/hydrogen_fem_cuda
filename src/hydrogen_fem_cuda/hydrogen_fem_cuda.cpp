...
namespace hydrogen_fem {
    ...
    Hydrogen_FEM::Hydrogen_FEM()
        :   hg_(Eigen::MatrixXf::Zero(NODE_TOTAL, NODE_TOTAL)),
            length_(ELE_TOTAL),
            ...
            ug_(Eigen::MatrixXf::Zero(NODE_TOTAL, NODE_TOTAL))
    {
    }
    ...
    float Hydrogen_FEM::do_run()
    {
        ...
        // 一般化固有値問題を解く
        auto const res = cuda::sygvd_cuda(hg_.rows(), hg_.data(), ug_.data());

        // エネルギー固有値を取得
        eigenval_ = std::get<0>(res);

        // 基底状態の固有関数（波動関数）を取得
        phi_ = std::get<1>(res);

        // 基底状態の固有ベクトル（波動関数）を取得
        phi_.resize(NODE_TOTAL);

        // 固有ベクトルの末尾を0にする
        phi_[NODE_TOTAL - 1] = 0.0;
    ...
    void Hydrogen_FEM::save_result() const
    {
        ...
        for (auto i = 0; i < NODE_TOTAL - 1; i++) {
            // 厳密な結果と比較
            std::fprintf(fp_eigenval.get(), "%d, %.14f, %.14f\n", i + 1, eigenval_[i], - 0.5 / static_cast<float>((i + 1) * (i + 1)));
        }
    }
    ...
    float Hydrogen_FEM::get_A_matrix_element(std::int32_t e, float le, std::int32_t p, std::int32_t q) const
    {
        auto const ed = static_cast<float>(e);
        switch (p) {
        case 0:
            switch (q) {
            case 0:
    ...
    float Hydrogen_FEM::get_B_matrix_element(std::int32_t e, float le, std::int32_t p, std::int32_t q) const
    {
        auto const ed = static_cast<float>(e);
        switch (p) {
        case 0:
    ...
    void Hydrogen_FEM::make_data()
    {
        ...
        auto const dr = (R_MAX - R_MIN) / static_cast<float>(ELE_TOTAL);
        for (auto i = 0; i <= ELE_TOTAL; i++) {
            // 計算領域を等分割
            node_r_glo_[i] = R_MIN + static_cast<float>(i) * dr;
        }
